...
describe('SystemSettingsPanel', () => {
  ...
  it('updates issuing center, select fields and nil-reasons checkbox', async () => {
    ...
    await user.selectOptions(
      screen.getByLabelText(/default on error behavior/i),
      'fail',
    );
    await user.selectOptions(screen.getByLabelText(/default log level/i), 'ERROR');

    const includeNilReasons = screen.getByLabelText(
      /include nil reasons/i,
    ) as HTMLInputElement;
    ...
    expect(
      (screen.getByLabelText(/default iwxxm version/i) as HTMLSelectElement).value,
    ).toBe('2023-1');
    expect(
      (screen.getByLabelText(/default on error behavior/i) as HTMLSelectElement).value,
    ).toBe('fail');
    expect(
      (screen.getByLabelText(/default log level/i) as HTMLSelectElement).value,
    ).toBe('ERROR');
  ...
  it('resets unsaved settings back to original values', async () => {
    ...
    const strictCheckbox = (await screen.findByLabelText(
      /strict validation/i,
    )) as HTMLInputElement;
    ...
    await waitFor(() => {
      ...
      expect(mockToast.info).toHaveBeenCalledWith(
        'Settings reset to last saved values',
      );
    });
  });
});

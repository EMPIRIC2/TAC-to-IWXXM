...
describe('AdminDashboard', () => {
  ...
  it('calls optional converter callback when File Converter button is clicked', async () => {
    ...
    render(
      <AdminDashboard {...defaultProps} onSwitchToConverter={onSwitchToConverter} />,
    );
  ...
  it('hides converter switch button when callback is not provided', () => {
    render(<AdminDashboard {...defaultProps} onSwitchToConverter={undefined} />);

    expect(
      screen.queryByRole('button', { name: /switch to file converter/i }),
  ...
  it('handles global and others logout scopes', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /logout options/i }));
    await user.click(
      screen.getByRole('button', { name: /sign out from other devices/i }),
    );

...
describe('Form Advanced UI Wrappers', () => {
  ...
  it('renders slider with two thumbs when value array has two items', () => {
    const { container } = render(
      <Slider value={[10, 80]} min={0} max={100} onValueChange={() => undefined} />,
    );
  ...
  it('toggles toggle-group single selection between items', async () => {
    ...
    render(
      <ToggleGroup type="single" defaultValue="metar" aria-label="Format select">
        <ToggleGroupItem value="metar" aria-label="METAR">
          METAR
        </ToggleGroupItem>
        <ToggleGroupItem value="taf" aria-label="TAF">
          TAF
        </ToggleGroupItem>
      </ToggleGroup>,
    );
  ...
  it('opens select content and selects item', async () => {
    ...
    expect(screen.getByRole('combobox', { name: 'IWXXM Version' })).toHaveTextContent(
      '2023-1',
    );
  });
  ...
  it('prevents selecting disabled select item', async () => {
    ...
    expect(
      screen.getByRole('combobox', { name: 'Disabled check', hidden: true }),

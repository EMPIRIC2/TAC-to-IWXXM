...
app.get('/make-server-2e3cda33/auth/check-approval/:userId', async (c) => {
  try {
    ...
    return c.json(
      {
        error: 'Internal server error checking approval status',
      },
      500,
    );
  }
...
app.post('/make-server-2e3cda33/auth/approve', async (c) => {
  try {
    ...
    return c.json(
      {
        error: 'Internal server error approving user',
      },
      500,
    );
  }
...
app.get('/make-server-2e3cda33/auth/pending-users', async (c) => {
  try {
    ...
    return c.json(
      {
        error: 'Internal server error getting pending users',
      },
      500,
    );
  }
...
app.post('/make-server-2e3cda33/auth/resend-verification', async (c) => {
  try {
    ...
    return c.json(
      {
        error: 'Internal server error resending verification email',
      },
      500,
    );
  }
...
app.put('/make-server-2e3cda33/auth/profile', async (c) => {
  try {
    ...
    const result = await auth.updateUserProfile(user.id, {
      username,
      email,
    });
    if (result.error) {
...
app.post('/make-server-2e3cda33/database/upload', async (c) => {
  try {
    ...
    return c.json(
      {
        error: 'Internal server error during database upload',
      },
      500,
    );
  }
...
app.get('/make-server-2e3cda33/database/uploads', async (c) => {
  try {
    ...
    const dbType = c.req.query('database') || 'both';
    ...
    return c.json(
      {
        error: 'Internal server error getting uploads',
      },
      500,
    );
  }
...
app.post('/make-server-2e3cda33/convert/metar-to-iwxxm', async (c) => {
  try {
    ...
    if (!metarContent || typeof metarContent !== 'string') {
      return c.json(
        {
          error: 'METAR content is required',
        },
        400,
      );
    }
...
async function verifyProjectJWT(jwt) {
  try {
    ...
    console.log(
      `   JWKS URL: https://ktvxijislbtgqapllmuk.supabase.co/auth/v1/.well-known/jwks.json`,
    );
    console.log(`   JWT prefix: ${jwt?.substring(0, 30)}...`);

    const verified = await jwtVerify(jwt, JWKS);
    const userId = verified.payload.sub;
    const email = verified.payload.email;
    const aal = verified.payload.aal;
    const exp = verified.payload.exp;
    const alg = verified.protected?.alg;
...
app.post('/make-server-2e3cda33/admin/reject-user', async (c) => {
  try {
    ...
    return c.json(
      {
        error: 'Internal server error rejecting user',
      },
      500,
    );
  }
...
app.get('/make-server-2e3cda33/admin/all-users', async (c) => {
  try {
    ...
    return c.json(
      {
        error: 'Internal server error getting all users',
      },
      500,
    );
  }
...
app.post('/make-server-2e3cda33/admin/toggle-admin', async (c) => {
  try {
    ...
    if (!userId || typeof isAdmin !== 'boolean') {
      return c.json(
        {
          error: 'User ID and isAdmin flag are required',
        },
        400,
      );
    }
    ...
    return c.json(
      {
        error: 'Internal server error toggling admin status',
      },
      500,
    );
  }
...
app.get('/make-server-2e3cda33/admin/settings', async (c) => {
  try {
    ...
    return c.json(
      {
        error: 'Internal server error getting settings',
      },
      500,
    );
  }
...
app.post('/make-server-2e3cda33/admin/settings', async (c) => {
  try {
    ...
    return c.json(
      {
        error: 'Internal server error saving settings',
      },
      500,
    );
  }

...
test.describe('Workflow: Auth And Admin Readiness', () => {
  test('startup, login, admin readiness, and converter transition are stable', async ({
    page,
  }) => {
    test.skip(
    ...
    await expect(
      page.getByRole('button', { name: /Open user preferences/i }),
    ).toBeVisible();
  });

  test('admin panel navigation loop remains stable across repeated transitions', async ({
    page,
  }) => {
    test.skip(
      !ADMIN_EMAIL || !ADMIN_PASSWORD,

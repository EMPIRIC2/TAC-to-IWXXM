"""T5.3 / TC-F20-005 - TAF + SPECI lint/convert + catalog GET smoke (H3-shaped).

Spec: docs/test-plan.md TC-F20-005; docs/api-contract.md §lint-issue-catalog;
execution-plan T5.3. In-process authenticated client (CI); live H3/H4-H5 reuse paths.
"""
...
TAF_TAC = (FIXTURES / "taf_basic.tac").read_text(encoding="utf-8").strip()
...
def smoke_client() -> Iterator[TestClient]:
    async def _auth_user() -> dict[str, str]:
        return {"sub": "f20-smoke-user", "aud": "test"}
...
@pytest.mark.parametrize(
    ("product", "tac", "root_hint"),
    [
        ("TAF", TAF_TAC, "taf"),
        ("SPECI", SPECI_TAC, "speci"),
    ],
)
def test_tc_f20_005_lint_and_convert_smoke(
    ...
    assert root_hint in xml.lower()


@pytest.mark.parametrize(
    ("product", "expected_code"),
    [
        ("taf", "MISSING_VALIDITY"),
        ("speci", "MISSING_CCCC"),
    ],
)
def test_tc_f20_005_catalog_get_smoke(
    smoke_client: TestClient,
    product: str,
    expected_code: str,
) -> None:

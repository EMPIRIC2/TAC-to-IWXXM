...
class TestSchematronDockerContainer:
    ...
    def test_validate_multiple_files(self, schema_path):
        ...
        for xml_file in xml_files:
            ...
            results.append({"file": xml_file.name, "returncode": result.returncode, "success": success})
    ...
    def test_docker_volume_mounting(self, test_xml_path, schema_path):
        ...
        result = subprocess.run(
            [
                ...
                "--rm",
                "-v",
                f"{test_xml_path.parent}:/input",
                "-v",
                f"{schema_path.parent.parent}:/schema",
                "metar-iwxxm-schematron:latest",
                f"/input/{test_xml_path.name}",
                "/schema/iwxxm/IWXXM/rule/iwxxm.sch",
            ],
            capture_output=True,
            text=True,
            timeout=30,
        ...
        assert "no such file or directory" not in result.stderr.lower(), f"Volume mounting failed: {result.stderr}"
...
class TestSchematronDockerProperties:
    ...
    def test_docker_image_size(self):
        ...
        result = subprocess.run(
            ["docker", "images", "metar-iwxxm-schematron:latest", "--format={{.Size}}"], capture_output=True, text=True
        )
    ...
    def test_docker_entrypoint(self):
        ...
        result = subprocess.run(["docker", "inspect", "metar-iwxxm-schematron:latest"], capture_output=True, text=True)

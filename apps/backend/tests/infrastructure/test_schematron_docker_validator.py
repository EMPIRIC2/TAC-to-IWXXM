...
from utilities.schematron_validator_docker import SchematronValidationResult, SchematronValidatorDocker
...
class TestSchematronValidationResult:
    ...
    def test_result_creation(self):
        ...
        result = SchematronValidationResult(
            valid=True, status="PASS", assertions_passed=5, assertions_failed=0, failed_constraints=[]
        )
    ...
    def test_result_with_failures(self):
        ...
        failures = [{"id": "assert-1", "test": "test-expr", "message": "Test failed"}]
        result = SchematronValidationResult(
            valid=False, status="FAIL", assertions_passed=3, assertions_failed=1, failed_constraints=failures
        )
...
class TestSchematronValidatorDocker:
    ...
    def validator(self):
        ...
        return SchematronValidatorDocker(schema_path=str(schema_path), version="2023-1")
    ...
    def test_validate_docker_error(self, mock_run, mock_temp, validator):
        ...
        mock_run.return_value = MagicMock(returncode=1, stdout="", stderr="Docker error: image not found")
...
class TestSchematronIntegration:
    ...
    def validator(self):
        ...
        return SchematronValidatorDocker(
            schema_path="/root/metar-to-IWXXM/schemas/iwxxm/IWXXM/rule/iwxxm.sch", version="2023-1"
        )
    ...
    def test_validate_multiple_samples(self, validator, test_data_dir):
        ...
        assert all(r["status"] in ["PASS", "FAIL", "ERROR", "UNKNOWN"] for r in results_summary), (
...
class TestSchematronEdgeCases:
    ...
    def validator(self):
        ...
        return SchematronValidatorDocker(schema_path=str(SCHEMA_PATH))

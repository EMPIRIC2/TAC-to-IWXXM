...
def test_parse_product_filter_valid_and_invalid() -> None:
    assert ws_router._parse_product_filter(None) is None
    assert ws_router._parse_product_filter("  ") is None
    assert ws_router._parse_product_filter("metar,speci") == [
        WorkSessionProduct.METAR,
        WorkSessionProduct.SPECI,
    ]
    with pytest.raises(HTTPException) as exc:
        ws_router._parse_product_filter("nope")
...
def test_work_session_service_dependency_factory() -> None:
    with patch.object(ws_router, "WorkSessionService") as mock_cls:
        ...
        service = ws_router.work_session_service({"sub": str(USER_ID)})
        assert service is mock_cls.return_value
        mock_cls.assert_called_once_with(str(USER_ID))

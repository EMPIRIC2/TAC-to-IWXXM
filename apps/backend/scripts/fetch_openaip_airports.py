...
def get_openaip_api_key() -> str:
    ...
    raise ValueError(
        "OPENAIP_API_KEY not found in environment or .env file. Set it in .env or export OPENAIP_API_KEY=<your_key>"
    )
...
def fetch_from_openaip(api_key: str, limit: int = 10000) -> Dict[str, dict]:
    ...
    while total_fetched < limit:
        params = {"limit": min(batch_size, limit - total_fetched), "skip": skip}
        ...
        for airport in items:
            ...
            if icao:
                airports[icao] = {
                    ...
                    "coordinates": {"latitude": airport.get("lat"), "longitude": airport.get("lon")},
                }
...
def main():
    ...
    parser.add_argument("--refresh", action="store_true", help="Force refresh from API even if cache exists")
    parser.add_argument("--limit", type=int, default=10000, help="Maximum number of airports to fetch (default: 10000)")
    parser.add_argument(
        "--cache-dir",
        type=Path,

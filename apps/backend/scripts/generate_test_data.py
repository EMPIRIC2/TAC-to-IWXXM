...
def generate_diverse_sample(generator: METARTestGenerator, count: int = 200):
    ...
    if api_key:
        print(f"   ✓ OpenAIP API key configured")
    else:
        print(f"   ℹ OpenAIP API key not found (will use local data only)")
...
def main():
    ...
    print(f"   - Test cases: diverse_sample_*.json")
    print(f"   - Regional samples: regional_*.json")
    print(f"   - Phenomenon coverage: phenomena_coverage_*.json")
    
    print(f"\n🧪 Next Steps:")
    print(f"   1. Run tests: pytest tests/test_dynamic_metar_generation.py -v")
    print(f"   2. View coverage report: cat {generator.cache_dir}/coverage_report.json")
    print(f"   3. Analyze failures: ls test-reports/dynamic-test-failures/")

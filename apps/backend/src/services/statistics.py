    ...
    async def log_translation(
        ...
        try:
            ...
            record = TranslationStatisticsModel(
                ...
                translation_status=translation_status.value if isinstance(translation_status, TranslationStatus) else translation_status,
        ...
        try:
            async with get_db_session() as session:
                ...
                overall_query = select(
                    ...
                    func.sum(
                        (TranslationStatisticsModel.translation_status == 'success').cast(int)
                    ).label('successful'),
                    func.sum(
                        (TranslationStatisticsModel.translation_status == 'failed').cast(int)
                    ).label('failed'),
                    func.sum(
                        (TranslationStatisticsModel.translation_status == 'partial').cast(int)
                    ).label('partial'),
                ...
                if include_error_details and total > 0:
                    error_query = select(
                        ...
                        and_(where_clause, TranslationStatisticsModel.translation_status != 'success')
                    ).group_by(
                        TranslationStatisticsModel.translation_status
                    ).order_by(
                        func.count(TranslationStatisticsModel.id).desc()

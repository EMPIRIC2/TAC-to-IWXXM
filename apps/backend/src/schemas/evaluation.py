...
class EvaluationRequest(BaseModel):
    ...
    hours: float = Field(
        default=1.5, description="Hours back to search for METAR data", ge=0.5, le=24, examples=[1.5, 3.0, 6.0]
    )
    large_airports_only: bool = Field(default=True, description="Only evaluate large airports (IATA code available)")
    scheduled_service_only: bool = Field(default=True, description="Only evaluate airports with scheduled service")
...
class EvaluationJobResponse(BaseModel):
    ...
    job_id: str = Field(
        ..., description="Unique job identifier (UUID)", examples=["job_550e8400e29b41d4a71662f7d6e6b1c0"]
    )
    status: JobStatus = Field(..., description="Current job status: pending, running, completed, failed")
    station_count: int = Field(..., description="Number of stations to evaluate", ge=0)
...
class JobSummaryStats(BaseModel):
    ...
    total: int = Field(..., description="Total stations evaluated", ge=0)
    passed: int = Field(..., description="Number of stations with passing comparison", ge=0)

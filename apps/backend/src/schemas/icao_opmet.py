...
class TranslationRecord(BaseModel):
    ...
    # Primary identifiers
    translation_id: str = Field(..., description="Unique UUID for this translation")
    timestamp: datetime = Field(..., description="Translation completion timestamp (ISO 8601)")

    # Airport and region
    icao_airport_code: str = Field(..., description="ICAO 4-letter airport identifier", min_length=4, max_length=4)
    icao_region: ICAORegion = Field(..., description="ICAO regional office jurisdiction")
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                ...
                "translations_by_region": {"NAM": 892, "EUR": 421, "APAC": 234},
                "translations_by_version": {"2025-2": 1501, "2023-1": 46},
            }
        }
    )

    # Time period
    period_start: datetime = Field(..., description="Statistics period start (ISO 8601)")
    period_end: datetime = Field(..., description="Statistics period end (ISO 8601)")

    # Overall counts
    total_translations: int = Field(..., description="Total number of translations in period", ge=0)
    successful_translations: int = Field(..., description="Successfully completed translations", ge=0)
    failed_translations: int = Field(..., description="Failed translations", ge=0)
    partial_translations: int = Field(default=0, description="Translations completed with warnings", ge=0)
...
class TranslationStatisticsRequest(BaseModel):
    ...
    include_error_details: bool = Field(default=False, description="Include detailed error analysis")

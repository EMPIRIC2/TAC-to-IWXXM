...
| R2 | Decision | F7 v1 includes **multi-product work sessions** (not UI panels only). F5 remains METAR/SPECI-only on its existing table; F7 gets a new sessions model for the seven products |
...
| Multi-product sessions | F7 / R2 | New session persistence for seven products (do **not** reuse F5 `metar_work_sessions` without explicit schema plan) |
...
- Extending **F5** My METARs to non-METAR without migrating to F7 sessions
...
| F5 sessions | METAR/SPECI only | R2: separate F7 multi-product sessions | Complementary, not F5 amendment |
...
5. **M5 — Multi-product sessions (F7 / R2)**  
   Schema + API + UI for seven-product sessions (new tables/routes — not quiet F5 extension); H4–H5 journeys.
...
| Gap | Owner stage |
|-----|-------------|
| Exact F7 sessions schema (tables, RLS, product enum columns) | 01 + 04 |

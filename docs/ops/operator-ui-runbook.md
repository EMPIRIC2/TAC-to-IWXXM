...
**Status (S050 / EV-042):** Operator Dissemination drawer sinks and Convert&Send are
**hidden** (`OPERATOR_DISSEMINATION_DESTINATIONS_ENABLED=false`). Operators use Convert,
Validate, mass Folder/Zip ingest (F33, auth required), and the work queue (UJ-052).

- Backend `/api/v1/dissemination/*` remains for harness / CI.
- Restore all destinations (DB + WIS2/EDIS/AMHS/SWIM/AFS) via

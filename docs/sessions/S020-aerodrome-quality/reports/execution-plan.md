...
| **Active task** | T3.4 (pending) |
| **Tasks** | 14 / 28 completed (through T3.3) |
...
| T3.5 | Test | Expand SPECI goldens annex3/iwxxm_us (S3); root `iwxxm:SPECI` | TC-F20-003 | T3.4 | pending |
...
| Task | Type | Description | Spec Source | Depends On | Status |
|------|------|-------------|-------------|------------|--------|
| T4.1 | Test | Common-rule fixtures (reportStatus/nilReasons/CRS/one-report) where lint applies | matrix C1; #735/#734 | T3.6 | pending |
| T4.2 | Code | Encode/defer C1 with rationale | F20; guidance | T4.1 | pending |
| T4.3 | Docs | COVERAGE_MATRIX F20 acc checklist; ISSUE_CATALOG regen | F20 acc4 | T4.2 | pending |

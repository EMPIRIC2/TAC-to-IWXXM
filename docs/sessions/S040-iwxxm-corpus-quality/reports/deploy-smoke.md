...
> Status: **PASS** (close approved `D-S040-close` = 1 on resume 2026-08-05)  
> Date: 2026-08-04 (initial) · **re-verified 2026-08-05** after S042 CD  
...
> Main CI tests: [30953419422](https://github.com/EMPIRIC2/TAC-to-IWXXM/actions/runs/30953419422) — Validate + all Test jobs **SUCCESS**; **Deploy hook FAIL** (Render suspended — expected post-DOKS cutover; later fixed by EV-034 #867/#868)  
> GHCR images (initial): `backend|frontend|worker:20260804214648-dfecba4` (+ `main-latest`)  
> **Current live (2026-08-05):** `20260805115809-d3f4bb9` via automated DOKS CD (EV-034)  
...
| TC-F32 / VONA convert | live `POST /api/v1/convert-bulletin` product=vona | **PASS** — `VolcanoObservatoryNoticeForAviation` |
| TC-EV032 FE | App chunk contains `vona_a7_1` / `vona-A7-1` | **PASS** (`App-BkEPMp_C.js` → `App-CNHsL_15.js` on re-verify) |

## Resume re-verify (2026-08-05)

| Check | Result |
|-------|--------|
| H1 `/health` | **PASS** (`tac2iwxxm_available: true`) |
| H0c CORS unit | **PASS** (6) |
| H4 CORS preflight (Origin app → API) | **PASS** (manual; `verify_connectivity.sh` broken under `set -u`) |
| H5 `/config.json` → API | **PASS** |
| VONA convert-bulletin | **PASS** root `VolcanoObservatoryNoticeForAviation` |
| FE `App-CNHsL_15.js` | **PASS** `vona_a7_1` + `vona-A7-1` present |

...
| **METAR (CA)** | ✅ MANOBS + Annex 3 ([dig](../mining/manobs-manair-ca-mining-notes.md)) | ✅ IWXXM 3.0.0 + `iwxxm-ca` emitter (EV-064–067) | ✅ Layered `ca_eccc` validate (EV-068/069): WMO 3.0.0 XSD+SCH + `metar-speci-ca.xsd` + code-ca + exchange | — |
| **TAF (CA)** | ✅ MANAIR 8th Ed. ([dig](../mining/manobs-manair-ca-mining-notes.md)) | ✅ IWXXM 3.0.0 + `taf-ca` NCLWS emitter (EV-064 M4) | ✅ WMO 3.0.0 XSD+SCH + `taf-ca.xsd` NCLWS probe (EV-069) | — |
| **AIRMET (CA)** | ✅ MANAIR GFA | ❌ convert not started (M5) | ✅ WMO 3.0.0 XSD+SCH + `airmet-ca.xsd` + code-ca (EV-069) | GFA convert M5 |

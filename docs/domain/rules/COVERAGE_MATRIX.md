...
| **METAR (CA)** | ✅ MANOBS + Annex 3 ([dig](../mining/manobs-manair-ca-mining-notes.md)) | ✅ IWXXM 3.0.0 + `iwxxm-ca` emitter (EV-064–067) | ✅ Layered `ca_eccc` validate (EV-068): WMO 3.0.0 XSD+SCH + `metar-speci-ca.xsd` · LWIS/SAWR substitution roots · TC-EV068-003 goldens | code-ca vocab ⚠ |
| **TAF (CA)** | ✅ MANAIR 8th Ed. ([dig](../mining/manobs-manair-ca-mining-notes.md)) | ✅ IWXXM 3.0.0 + `taf-ca` NCLWS emitter (EV-064 M4) | ⚠ WMO 3.0.0 XSD+SCH · `taf-ca.xsd` product gate gaps (TC-EV068-002 backlog) | EV-068 `#1035` deepen |
| **AIRMET (CA)** | ✅ MANAIR GFA | ❌ convert not started (M5) | ⚠ aspirational — EV-068 `#1035` | GFA phenomena codes — M5 |

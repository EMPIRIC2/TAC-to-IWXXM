...
- TAC detector **python residuals** (same class as decode enrichment): R1–R5/R8 hatches, `hatch_taf`, `hatch_sigmet` (`sigmet-core`), `hatch_airmet`, `hatch_vaa`, `hatch_tca`. **M2/M4 decision:** these do **not** block detectors `full` when packs are YAML DSL SoT and residuals are named python hatches only.
...
- Decode residuals: catch-all `token` on token-stream packs; VAA/TCA label packs keep free-text in label values — does **not** block decode `full`.

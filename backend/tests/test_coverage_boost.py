...
from api import app, verify_supabase_token
...
class TestAPIConversionErrorPaths:
    ...
    def test_convert_manual_text_conversion_error(self, client):
        ...
        with patch('api.convert_metar_tac', side_effect=ConversionError("Test error")):
    ...
    def test_convert_file_conversion_error_path(self, client):
        ...
        with patch('api.convert_metar_tac', side_effect=ConversionError("Parse error")):
            response = client.post(
                "/api/v1/convert",
    ...
    def test_convert_file_generic_exception_path(self, client):
        ...
        with patch('api.convert_metar_tac', side_effect=RuntimeError("Unexpected error")):
            response = client.post(
                "/api/v1/convert",
                files={"files": ("test.txt", test_file, "text/plain")}
            )
    ...
    def test_zip_manual_conversion_error(self, client):
        ...
        with patch('api.convert_metar_tac', side_effect=ConversionError("Zip error")):
            response = client.post(
                "/api/v1/convert-zip",
                data={"manual_text": "METAR KJFK 231751Z"}
            )
    ...
    def test_zip_empty_file_error(self, client):
        ...
        # Empty file should trigger error path
        assert response.status_code == 400
    ...
    def test_zip_file_conversion_error(self, client):
        ...
        with patch('api.convert_metar_tac', side_effect=ConversionError("Conversion failed")):
            response = client.post(
                "/api/v1/convert-zip",
                files={"files": ("test.txt", test_file, "text/plain")}
            )
    ...
    def test_zip_file_generic_exception(self, client):
        ...
        with patch('api.convert_metar_tac', side_effect=RuntimeError("Unexpected")):
            response = client.post(
                "/api/v1/convert-zip",
                files={"files": ("test.txt", test_file, "text/plain")}
            )
    ...
    def test_zip_no_valid_conversions(self, client):
        ...
        with patch('api.convert_metar_tac', side_effect=ConversionError("All fail")):
            response = client.post(
                "/api/v1/convert-zip",
                data={"manual_text": "INVALID"}
            )
            assert response.status_code == 400
            assert "No valid conversions" in response.json()["detail"]["message"]

    def test_zip_response_headers(self, client):
        """Test zip StreamingResponse headers (line 180)."""
        with patch('api.convert_metar_tac', return_value="<iwxxm>test</iwxxm>"):
            response = client.post(
                "/api/v1/convert-zip",
                data={"manual_text": "METAR KJFK 231751Z"}
            )
...
class TestConversionUtilityEdgeCases:
    ...
    def test_load_aerodrome_db_docker_path(self):
        ...
        from utilities.conversion import _load_aerodrome_db
    ...
    def test_lookup_aerodrome_empty_parts(self):
        ...
        from utilities.conversion import _lookup_aerodrome

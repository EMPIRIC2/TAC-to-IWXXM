...
def test_api_with_validation():
        ...
        if response.status_code == 200:
            ...
            if result['results']:
                ...
                if "BURI RAM AIRPORT" in xml:
                    print(f"  ✓ Airport name capitalized correctly")
                else:
                    print(f"  ✗ Airport name not capitalized")
    ...
    try:
        ...
        if response.status_code == 400:
            print(f"✓ Invalid METAR correctly rejected")
            result = response.json()
            if 'detail' in result:
            ...
            if result.get('failed', 0) > 0:
                print(f"✓ Invalid METAR caught during processing")
                print(f"  Errors: {result.get('errors', [])}")
            else:
                print(f"✗ Invalid METAR was not caught")
        else:

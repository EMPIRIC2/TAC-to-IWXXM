...
class TestCodeListsToSchematronFetchAndDownload:
    ...
    def test_fetchLocalCopy_success(self, tmp_path):
        ...
        with patch("validation.codeListsToSchematron.requests.get") as mock_get:
            ...
            mock_get.side_effect = [mock_list_response, mock_file_response, mock_file_response]

            codeListsToSchematron.fetchLocalCopy("http://schemas.wmo.int/iwxxm/3.0", "xsd", str(dest_dir))
    ...
    def test_fetchLocalCopy_bad_status_code(self, tmp_path):
        ...
        with patch("validation.codeListsToSchematron.requests.get", return_value=mock_response):
            ...
            codeListsToSchematron.fetchLocalCopy("http://schemas.wmo.int/iwxxm/bad", "xsd", str(dest_dir))
    ...
    def test_download_codelist_success(self, tmp_path):
        ...
        with patch("validation.codeListsToSchematron.requests.get", return_value=mock_response):
            with patch("validation.codeListsToSchematron.os.path.join", return_value=str(tmp_path / "test.rdf")):
                with patch("builtins.open", mock_open()) as mock_file:
                    codeListsToSchematron.download_codelist("http://codes.wmo.int/common/nil", str(tmp_path))
    ...
    def test_download_codelist_failure(self, tmp_path):
        ...
        with patch("validation.codeListsToSchematron.requests.get", return_value=mock_response):
            ...
            codeListsToSchematron.download_codelist("http://codes.wmo.int/bad/url", str(tmp_path))
    ...
    def test_download_codelist_unicode_encode_error(self, tmp_path):
        ...
        with patch("validation.codeListsToSchematron.requests.get", return_value=mock_response):
            with patch("validation.codeListsToSchematron.os.path.join", return_value=str(tmp_path / "test.rdf")):
                with patch("builtins.open", mock_open()) as mock_file:
                    # Make write raise UnicodeEncodeError
                    mock_file.return_value.write.side_effect = [UnicodeEncodeError("ascii", "", 0, 1, "error"), None]
...
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">
    ...
    def test_parseLocalCodeListFile(self):
        ...
        result = codeListsToSchematron.parseLocalCodeListFile("http://codes.wmo.int/common/nil")
        assert result == "codes.wmo.int-common-nil.rdf"

        result = codeListsToSchematron.parseLocalCodeListFile("http://codes.wmo.int/49-2/AerodromeRecentWeather")

...
def main(cmdargs):
    ...
    cwd = os.getcwd()
    if not os.path.isfile(os.path.join(cwd, "bin", "crux-1.3-all.jar")) or not os.path.isdir(os.path.join(cwd, "externalSchemas")):
    ...
    if cmdargs.fetch:
        ...
        if not os.path.isfile(os.path.join(schemaPath, "iwxxm.xsd")) or not os.path.isfile(os.path.join(schematronPath, "iwxxm.sch")):
...
def validate_xml_files(cmdargs):
    ...
    print("Validating all XML files in %s with IWXXM XML schemas and schematron (version: %s)" % (cmdargs.directory, cmdargs.version))
...
if __name__ == "__main__":
    ...
    parser.add_argument("-f", "--fetch", help="fetch files from WMO Code Registry and WMO schema site", action="store_true", default=False)
    parser.add_argument(
        "-u", "--useInternet", help="when checking GML links, query WMO Code Registry for validation", action="store_true", default=False
    )
    parser.add_argument("--noGMLChecks", help="skip GML link checking", action="store_true", default=False)
    parser.add_argument("-k", "--keep", help="do not delete catalog file when validation finishes", action="store_true", default=False)
    parser.add_argument("-v", "--version", help="IWXXM version major.minor number to validate against, default '2023-1'", type=str, default="2023-1")

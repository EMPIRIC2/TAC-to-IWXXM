...
def check_GML_references(examplesDirectory, iwxxm_version, internet=False):
    ...
    for docFilename in xmlDocs:
        ...
        gmlIds = set([x.get("{http://www.opengis.net/gml/3.2}id") for x in root.iterfind(".//*[@{http://www.opengis.net/gml/3.2}id]")])
    ...
    if additionalMsg:
        print("Examine WARNED references for correctness. If valid, create RDF files for them and place in schematron directory")

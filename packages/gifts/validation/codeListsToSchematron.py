...
def run(cmdargs):
    ...
    if "http://codes.wmo.int/49-2/AerodromePresentOrForecastWeather" in typeToCodeList.values():
        ...
        if not os.path.isfile(os.path.join(schematronPath, deslink)):
            try:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--version", "-v", help="IWXXM version major.minor number to validate against", type=str, default="3.0")

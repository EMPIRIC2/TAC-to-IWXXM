...
class Annex3(tpg.Parser):
    ...
    def seastate(self, s):
        ...
        self._metar["seastate"] = {"str": s, "index": self.index(), "seaSurfaceTemperature": seatemp, stateType: result.group("value")}
    ...
    def rwystate(self, s):  # pragma: no cover
        ...
        result = rePattern.match(s)
        try:
            self._metar["rwystate"].append({"str": s, "index": self.index(), "runway": result.group(1), "state": result.group(2)})
        except KeyError:
            self._metar["rwystate"] = [{"str": s, "index": self.index(), "runway": result.group(1), "state": result.group(2)}]
    ...
    def timeBoundary(self, s):
        ...
        try:
            self._trend["ttime"].update({s[:2]: time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime(calendar.timegm(tuple(tms))))})
        except KeyError:
            self._trend.update({"ttime": {s[:2]: time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime(calendar.timegm(tuple(tms))))}})

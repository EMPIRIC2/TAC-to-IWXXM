    ...
    def __call__(self, tac):
        ...
        try:
            ...
            if len(self._expected):
                err_msg = "Expecting %s group(s) " % " or ".join([self._tokenInEnglish.get(x, x) for x in self._expected])
            else:
                ...
                if des.noImpliedCAVOKCondition:
                    ...
                    if vsby is None:
                        ...
                        if des.emitImpliedCAVOKConditionMessage:
                            self._Logger.info("TAF %s: Added >10km visibility to forecast group on line %s, column %s" % (icaoID, line, column))
                    else:
                        self.sky("NSC")
                        if des.emitImpliedCAVOKConditionMessage:
                            self._Logger.info("TAF %s: Added NSC to forecast group on line %s, column %s" % (icaoID, line, column))

...
class simpleGUI(object):
    def __init__(self):
        ...
        self.encoders = []
        self.encoders.append((re.compile(r"^S(A|P)[A-Z][A-Z]\d\d\s+[A-Z]{4}\s+\d{6}", re.MULTILINE), gifts.METAR.Encoder(aerodromes)))
        self.encoders.append((re.compile(r"^F(C|T)[A-Z][A-Z]\d\d\s+[A-Z]{4}\s+\d{6}", re.MULTILINE), gifts.TAF.Encoder(aerodromes)))
...
if __name__ == "__main__":
    ...
    if "--test" in sys.argv:
        ...
        encoders = []
        encoders.append((re.compile(r"^S(A|P)[A-Z][A-Z]\d\d\s+[A-Z]{4}\s+\d{6}", re.MULTILINE), gifts.METAR.Encoder(aerodromes), "METAR"))
        encoders.append((re.compile(r"^F(C|T)[A-Z][A-Z]\d\d\s+[A-Z]{4}\s+\d{6}", re.MULTILINE), gifts.TAF.Encoder(aerodromes), "TAF"))
        encoders.append((re.compile(r"FK\w\w\d\d\s+[A-Z]{4}\s+\d{6}", re.MULTILINE), gifts.TCA.Encoder(), "TCA"))
        encoders.append((re.compile(r"FV\w\w\d\d\s+[A-Z]{4}\s+\d{6}", re.MULTILINE), gifts.VAA.Encoder(), "VAA"))

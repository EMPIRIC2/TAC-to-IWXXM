...
| Schematron | **native** `validate_iwxxm`                            | Evaluates Schematron after **dsdl→dml** NS remap + XPath2 `if()` / comparison / `number(text())` rewrites + Observation-2 / Report-9 stand-ins + `document()` codelist stand-in (with `xlink:href`→`href` mirror). Residual XPath2 (extension-size, nilReason `document()`, …) → soft `SCHEMATRON_XPATH_UNSUPPORTED` | does **not** emit `SCHEMATRON_SKIPPED`       |

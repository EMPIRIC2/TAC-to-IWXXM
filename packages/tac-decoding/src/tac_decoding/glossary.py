...
(``decode_glossary.yaml`` / ``TAC_DECODING_GLOSSARY_PATH`` or legacy
``TAC2IWXXM_DECODE_GLOSSARY_PATH`` wins on conflict).
...
def load_glossary() -> dict[str, str]:
    ...
    env_path = os.environ.get(_ENV_PATH, "").strip() or os.environ.get(_LEGACY_ENV_PATH, "").strip()
    if env_path:

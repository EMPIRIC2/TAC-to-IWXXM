...
TEMPLATES = [
    (
        "confirmation",
        "frontend/templates/authentication/01-confirmation.md",
        "Confirm your email address",
    ),
    (
        "magic_link",
        "frontend/templates/authentication/02-magic-link.md",
        "Your magic link to sign in",
    ),
    (
        "recovery",
        "frontend/templates/authentication/03-password-reset.md",
        "Reset your password",
    ),
    (
        "email_change",
        "frontend/templates/authentication/05-email-changed.md",
        "Email address changed",
    ),
]


def extract_html_from_template(file_path: str) -> str | None:
...
def upload_template(
    access_token: str, project_id: str, template_type: str, subject: str, html: str
) -> bool:
...
def main():
    ...
    parser.add_argument(
        "--access-token", required=True, help="Supabase Management API access token"
    )
    ...
    for template_type, subject, html in templates_data:
        if upload_template(
            args.access_token, args.project_id, template_type, subject, html
        ):
            success_count += 1
...
if __name__ == "__main__":
    ...
    print(
        "configured manually in Supabase Dashboard → Authentication → Email Templates"
    )

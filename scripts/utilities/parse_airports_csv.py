...
def main() -> None:
    ...
    print(
        f"  With IATA codes: {airports_with_iata} ({airports_with_iata / len(airports) * 100:.1f}%)"
    )
    print(
        f"  With coordinates: {airports_with_coords} ({airports_with_coords / len(airports) * 100:.1f}%)"
    )
    ...
    print("\nAirport types:")
    for airport_type, count in sorted(

...
class TestM7PlaywrightMonorepoConfig:
    ...
    def test_playwright_uses_monorepo_dev_stack(self, playwright_config: str) -> None:
        ...
        assert (
            "PLAYWRIGHT_API_BASE_URL=${process.env.PLAYWRIGHT_API_BASE_URL || DEFAULT_API_BASE_URL}"
            in playwright_config
        )

    def test_playwright_defaults_to_local_dev_ports(
        self, playwright_config: str
    ) -> None:
        assert "http://localhost:18000" in playwright_config
        assert "http://localhost:18001" in playwright_config
        assert "http://localhost:5173" in playwright_config

    def test_playwright_normalizes_stale_local_base_url_when_webserver_runs(
        self, playwright_config: str
    ) -> None:
        assert "const requestedBaseUrl" in playwright_config
        assert "isLegacyLocalViteUrl" in playwright_config
        assert "http://localhost:5173" in playwright_config
        assert "? DEFAULT_FRONTEND_URL" in playwright_config

    def test_global_setup_waits_for_merged_backend_only(
        self, global_setup: str
    ) -> None:
        assert "http://localhost:18001" in global_setup
    ...
    def test_auth_integration_spec_targets_merged_api(self) -> None:
        spec = (APPS_E2E / "auth-service-integration.e2e.spec.ts").read_text(
            encoding="utf-8"
        )
        assert "playwrightApiBaseUrl" in spec
        assert "const API_BASE_URL = playwrightApiBaseUrl()" in spec

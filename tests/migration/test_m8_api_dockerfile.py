...
class TestM8ApiDockerfileMonorepoContext:
    ...
    def test_dockerfile_copies_monorepo_workspace_members(self, dockerfile_text: str) -> None:
        for fragment in (
            "apps/backend",
            "packages/auth",
            "packages/gifts",
    ...
    def test_dockerfile_does_not_reference_legacy_layout(self, dockerfile_text: str) -> None:
    ...
    def test_dockerfile_exposes_healthcheck_and_port(self, dockerfile_text: str) -> None:
        assert "HEALTHCHECK" in dockerfile_text
        assert "/health" in dockerfile_text
        assert "8000" in dockerfile_text

    def test_dockerfile_runs_backend_module_entrypoint(self, dockerfile_text: str) -> None:
        assert 'python", "-m", "src"' in dockerfile_text or "python -m src" in dockerfile_text
